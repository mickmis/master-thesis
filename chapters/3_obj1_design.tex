...
% generateConstraintFromSelectedNode: dropmode/query: from the saved queries -- dropmode/patient set: simply expose field, as the others // like now it won't be searchable, but queryable through importation of the set
...
% todo: data types list and constraint: because that defines how the method is implemented
% \begin{itemize}
...
%     \item \emph{Patient subselection}: todo: how different from patient set / about selection of dimension 
%     \item \emph{True}: everything (not supported)
%     \item \emph{Relation / Pedigree}:???
% \end{itemize}
% todo: isObservationDate, 
...
% todo: implementation of the models for each constraint (+ each data type)
% todo: PATIENT_SET a data type? 
% todo: add how the where clause is generated + an example
...
% TODO: not necessarily need to map to preserve features? just go ahead with pic sure supported stuff
% meaning the special fields and all should be seen from the resource point of view (e.g. study, trial-visit, relation-pedigree)
% in tree have a special field for displaying things in UI rather than in tree? 
% although still have the special cases for constraints 
% take notes of features (tmp) removed (checkboxes for query things)
% 
